...
def run_cov(mode: Literal["s_mu", "legendre_projected", "legendre_accumulated"],
    ...
    if (2 * (max(s_edges) + xi_cut_s) > boxsize):
        ...
        # for the true distance to become smaller than xicutoff given the wrapping is wrong, the "naive" separation should exceed boxsize - xicutoff
        # thus the final condition for the issue is 2*(xicutoff + rmax) > boxsize (which is necessarily true if xicutoff > boxsize/2 as rmax > 0)
