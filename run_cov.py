...
tracers = ['LRG'] * 3
zs = [[0.4, 0.6], [0.6, 0.8], [0.8, 1.1]]
z2s = [[0.4, 1.1]] * 3
sms = [10] * 3
rectypes = ['MGrecsym'] * 3
feffs = [0.762, 0.817, 0.864]
# need 6 jobs in this array

tlabels = [tracers[id]] # tracer labels for filenames
sm = sms[id] # smoothing scale in Mpc/h
rectype = rectypes[id] # reconstruction type
feff = feffs[id] # effective growth rate, for reconstruction
...
z_min2, z_max2 = z2s[id] # for filenames
...
if convert_cf:
    ...
    split_above = 20
    pycorr_filenames = [[check_path(f"/global/cfs/cdirs/desi/survey/catalogs/Y1/LSS/iron/LSScats/v0.1/blinded/recon_sm{sm}/{z_min2}z{z_max2}_feff{feff}/xi/smu/{f'njack{njack}/' if jackknife else ''}allcounts_{corlabel}_{rectype}_{reg}_{z_min}_{z_max}_default_FKP_lin_njack{njack if jackknife else 0}_nran{nrandoms}_split{split_above}.npy")] for corlabel in tlabels]
...
if jackknife or count_ndata:
    data_ref_filenames = [check_path(f"/global/cfs/cdirs/desi/survey/catalogs/Y1/LSS/iron/LSScats/v0.1/blinded/recon_sm{sm}/{z_min2}z{z_max2}_feff{feff}/{tlabel}_{reg}_clustering.{rectype}.dat.fits") for tlabel in tlabels] # only for jackknife reference or ndata backup, has to have rdz contents
    assert len(data_ref_filenames) == ntracers, "Need reference data for all tracers"
input_filenames = [[check_path(f"/global/cfs/cdirs/desi/survey/catalogs/Y1/LSS/iron/LSScats/v0.1/blinded/recon_sm{sm}/{z_min2}z{z_max2}_feff{feff}/{tlabel}_{reg}_{i}_clustering.{rectype}.ran.fits") for i in range(nrandoms)] for tlabel in tlabels] # random filenames
...
outdir = os.path.join(f"recon_sm{sm}", "_".join(tlabels + [rectype, reg]) + f"_z{z_min}-{z_max}_feff{feff}") # output file directory
