...
- ``-weighted_norm``: Flag indicating to use the weighted/effective number of random particles (sum(w)^2/sum(w^2), invariant to weight rescaling) instead of simple counting (default). Should match the way the -norm value was computed for the data galaxies externally.
